...
<template>
    <div :style="{width:tableWidth}" class='autoTbale'>
        <table class="table table-bordered" id='hl-tree-table' align="center">
            <thead>
            <tr>
            ...
            <tr v-for="(item,index) in initItems" :key="item.id" v-show="show(item)"
                :class="{'active':index ===activeIndex}" @click="RowClick(item,$event,index)">
                <td v-for="(column,snum) in columns" :key="column.key" :style=tdStyle(column)>
                    <label v-if="column.type === 'selection'">
                        <input type="checkbox" :value="item.id" v-model="checkGroup">
                    </label>
                    <div v-if="column.type === 'action'&&item.children ==null">
                        <i-button :type="action.type" size="small"
                                  @click.stop="actionClick(item,$event,index,action.text)"
...
<script>
    export default {
        ...
        methods: {
            ...
            // 点击右侧的一个按钮
            actionClick(data, event, index, text) {
                this.tableShow = true;
                this.activeParentFunId = data.Id;
                let result = this.makeData(data)
            },
            hideModal() {
            ...
            // 点击事件 返回数据处理
            makeData(data) {
                const t = (data !== null && data !== '' && data !== 'undefined') && typeof(data);
                let o;
                if (t === 'array') {
                    o = [];
                } else if (t === 'object') {
